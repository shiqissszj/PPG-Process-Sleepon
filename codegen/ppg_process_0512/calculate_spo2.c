...
float calculate_spo2(float inputR)
{
  ...
  /*  segmentedCoeffs = [111.681029336, -32.427245497, 0.840888038,
   * -27.951537941, 1.028421640, -38.543077665]; */
  /*  Spo2 = (inputR <= segmentedCoeffs(3)).*(segmentedCoeffs(1) +
   * segmentedCoeffs(2) * inputR) + ... */
  /*      (inputR > segmentedCoeffs(3) & inputR <=
   * segmentedCoeffs(5)).*(segmentedCoeffs(1) + segmentedCoeffs(2) *
   * segmentedCoeffs(3) + segmentedCoeffs(4) * (inputR - segmentedCoeffs(3))) +
   * ... */
  /*      (inputR > segmentedCoeffs(5)).*(segmentedCoeffs(1) +
   * segmentedCoeffs(2) * segmentedCoeffs(3) + segmentedCoeffs(4) *
   * (segmentedCoeffs(5) - segmentedCoeffs(3)) + segmentedCoeffs(6) * (inputR -
   * segmentedCoeffs(5))); */
  /*  Current calibration from R_SpO2_scatter_for_calibration.m */
  return fmaxf(fminf((-1.75535083F * (inputR * inputR) + -29.573164F * inputR) +
                         110.586418F,
                     100.0F),
               65.0F);
}
