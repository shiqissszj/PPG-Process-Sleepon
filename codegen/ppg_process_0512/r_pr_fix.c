               ...
               float *outputConfidenceR)
{
  ...
  if ((!rtIsInfF(inputPR)) && (!rtIsNaNF(inputPR)) && (inputPR >= 35.0F) &&
      (inputPR <= 220.0F)) {
    inputPRUsable = true;
  } else {
    inputPRUsable = false;
  }
  if ((!prSeeded) && inputPRUsable && (confidenceG > 0.35F)) {
    for (i = 0; i < 25; i++) {
  ...
  } else if (reliableCounterG > 0U) {
    tmp_size = (int)reliableCounterG;
    memcpy(&b_tmp_data[0], &fixBufferPR[0],
           (unsigned int)tmp_size * sizeof(float));
    fallbackPR = d_mean(b_tmp_data, (int)reliableCounterG);
  } else {
    fallbackPR = 60.0F;
  }
  ...
  if (confidenceG < 0.55F) {
    /*  fix the R and PR value */
    if (inputPRUsable && (confidenceG > 0.35F)) {
      confidenceBlend = (confidenceG - 0.35F) / 0.200000018F;
      fallbackPR =
          fallbackPR * (1.0F - confidenceBlend) + inputPR * confidenceBlend;
    } else if (inputPRUsable && (reliableCounterG == 0U)) {
      fallbackPR = inputPR;
    }

    /*  keep the original value */
  } else if (inputPRUsable) {
    fallbackPR = inputPR;
  ...
  if (prShouldUpdateHistory) {
    ...
    reliableCounterG = qY;
    fixBufferPR[(int)fixBufferIndexG - 1] = fallbackPR;
