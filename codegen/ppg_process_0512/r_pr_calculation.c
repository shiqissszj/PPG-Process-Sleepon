...
static float refine_pr_local_autocorr(const emxArray_real32_T *inputSignal,
                                      float coarsePR, float trackedPR);
...
static float refine_pr_local_autocorr(const emxArray_real32_T *inputSignal,
                                      float coarsePR, float trackedPR)
{
  emxArray_real32_T *b_inputSignal;
  emxArray_real32_T *r;
  emxArray_real32_T *r1;
  emxArray_real_T *corrValues;
  double *corrValues_data;
  const float *inputSignal_data;
  float refinedPR;
  float *b_inputSignal_data;
  int idx;
  int lag;
  inputSignal_data = inputSignal->data;
  emxInit_real_T(&corrValues);
  emxInit_real32_T(&b_inputSignal);
  emxInit_real32_T(&r);
  emxInit_real32_T(&r1);
  if (coarsePR <= 0.0F) {
    refinedPR = -1.0F;
    ...
    double maxCorrValue;
    float lagMarginRatio;
    float minLag;
    int bestIndex;
    int i;
    int loop_ub;
    refinedPR = 3000.0F / coarsePR;
    lagMarginRatio = 0.08F;
    if (trackedPR > 0.0F) {
      lagMarginRatio = 0.06F;
    }
    minLag = fmaxf(2.0F, roundf(refinedPR * (1.0F - lagMarginRatio)));
    refinedPR =
        fmaxf(minLag + 1.0F, roundf(refinedPR * (lagMarginRatio + 1.0F)));
    /*  initialization */
    /*  len = length(inputSig1);  */
    /*  corrValues = single(-Inf);  */
    /*  maxLagValueidx = int32(0);  */
    loop_ub = (int)((refinedPR - minLag) + 1.0F);
    bestIndex = corrValues->size[0];
    corrValues->size[0] = loop_ub;
    emxEnsureCapacity_real_T(corrValues, bestIndex);
    corrValues_data = corrValues->data;
    for (idx = 0; idx < loop_ub; idx++) {
      corrValues_data[idx] = 0.0;
    }
    /*  calculate the corr for each possible delay */
    i = (int)(refinedPR + (1.0F - minLag));
    for (lag = 0; lag < i; lag++) {
      int b_loop_ub;
      int c_loop_ub;
      int i1;
      int i2;
      lagMarginRatio = minLag + (float)lag;
      if (lagMarginRatio + 1.0F > inputSignal->size[0]) {
        bestIndex = 0;
        loop_ub = 0;
      } else {
        bestIndex = (int)(lagMarginRatio + 1.0F) - 1;
        loop_ub = inputSignal->size[0];
      }
      refinedPR = (float)inputSignal->size[0] - lagMarginRatio;
      if (refinedPR < 1.0F) {
        b_loop_ub = 0;
      } else {
        b_loop_ub = (int)refinedPR;
      }
      if (lagMarginRatio + 1.0F > inputSignal->size[0]) {
        i1 = 0;
        i2 = 0;
      } else {
        i1 = (int)(lagMarginRatio + 1.0F) - 1;
        i2 = inputSignal->size[0];
      }
      c_loop_ub = loop_ub - bestIndex;
      if (c_loop_ub == b_loop_ub) {
        loop_ub = b_inputSignal->size[0];
        b_inputSignal->size[0] = c_loop_ub;
        emxEnsureCapacity_real32_T(b_inputSignal, loop_ub);
        b_inputSignal_data = b_inputSignal->data;
        for (idx = 0; idx < c_loop_ub; idx++) {
          b_inputSignal_data[idx] =
              inputSignal_data[bestIndex + idx] * inputSignal_data[idx];
        }
        bestIndex = i2 - i1;
        loop_ub = r->size[0];
        r->size[0] = bestIndex;
        emxEnsureCapacity_real32_T(r, loop_ub);
        b_inputSignal_data = r->data;
        for (idx = 0; idx < bestIndex; idx++) {
          refinedPR = inputSignal_data[i1 + idx];
          b_inputSignal_data[idx] = refinedPR * refinedPR;
        }
        loop_ub = r1->size[0];
        r1->size[0] = b_loop_ub;
        emxEnsureCapacity_real32_T(r1, loop_ub);
        b_inputSignal_data = r1->data;
        for (idx = 0; idx < b_loop_ub; idx++) {
          refinedPR = inputSignal_data[idx];
          b_inputSignal_data[idx] = refinedPR * refinedPR;
        }
        corrValues_data[(int)((lagMarginRatio - minLag) + 1.0F) - 1] =
            sum(b_inputSignal) / (sqrtf(sum(r)) * sqrtf(sum(r1)));
      } else {
        binary_expand_op(corrValues, lagMarginRatio, minLag, inputSignal,
                         bestIndex, loop_ub - 1, b_loop_ub - 1, i1, i2 - 1,
                         b_loop_ub - 1);
        corrValues_data = corrValues->data;
      }
    ...
    maxCorrValue = c_maximum(corrValues);
    /*  normalized the corr (optional) */
    /*  normFactor = sqrt(sum(inputSig1.^2) * sum(inputSig2.^2)); */
    /*  maxCorr = maxCorr / normFactor; */
    if ((corrValues->size[0] == 0) || (maxCorrValue < 0.34999999403953552)) {
      refinedPR = coarsePR;
    } else {
      double bestValue;
      bestIndex = 1;
      bestValue = corrValues_data[0];
      loop_ub = corrValues->size[0];
      for (idx = 0; idx <= loop_ub - 2; idx++) {
        double d;
        d = corrValues_data[idx + 1];
        if (d > bestValue) {
          bestValue = d;
          bestIndex = idx + 2;
        }
      }
      refinedPR = 3000.0F / ((minLag + (float)bestIndex) - 1.0F);
      if ((trackedPR > 0.0F) &&
          (fabsf(refinedPR - trackedPR) > fabsf(coarsePR - trackedPR) + 4.0F) &&
          (maxCorrValue < 0.550000011920929)) {
        refinedPR = coarsePR;
      } else {
        lagMarginRatio =
            0.35F * fminf(fmaxf(((float)maxCorrValue - 0.35F) / 0.25F, 0.0F),
                          1.0F) +
            0.25F;
        refinedPR =
            coarsePR * (1.0F - lagMarginRatio) + refinedPR * lagMarginRatio;
      }
    }
  }
  emxFree_real32_T(&r1);
  emxFree_real32_T(&r);
  emxFree_real32_T(&b_inputSignal);
  emxFree_real_T(&corrValues);
  return refinedPR;
}
                       ...
                       float *outputConfidenceR, float *outputConfidenceG)
{
  ...
  if (loop_ub < 2) {
    ...
    if (jumpLimit < 2) {
      ...
      PR = refine_pr_local_autocorr(
          ppgExpandedG,
          estimate_tail_tracked_pr(peakLocG0_data, jumpLimit, validHistoryCount,
                                   trackedPR),
          trackedPR);
      if (PR < 35.0F) {
        guard1 = true;
