...
static int b_thirdOfFive(const float v_data[], int ia, int ib);

static int c_thirdOfFive(const float v[10], int ia, int ib);

static int thirdOfFive(const float v[30], int ia, int ib);
...
float b_quickselect(float v_data[], int n, int vlen, int *nfirst, int *nlast)
{
  float vn;
  int k;
  if (n > vlen) {
      ...
      if (guard1) {
        ...
        if (isslow) {
          while (ilast > 1) {
            ...
            for (k = 0; k < i; k++) {
              ipiv = (ia + k * 5) + 1;
              ipiv = b_thirdOfFive(v_data, ipiv, ipiv + 4) - 1;
              ifirst = ia + k;
              vn = v_data[ifirst];
            ...
            if (b_nlast > 0) {
              ...
              ipiv = b_thirdOfFive(v_data, ipiv, (ipiv + b_nlast) - 1) - 1;
      ...
      if (guard1) {
        ...
        if (isslow) {
          while (ilast > 1) {
            ...
            for (k = 0; k < i; k++) {
              ipiv = (ia + k * 5) + 1;
              ipiv = c_thirdOfFive(v, ipiv, ipiv + 4) - 1;
              ifirst = ia + k;
              vn = v[ifirst];
              v[ifirst] = v[ipiv];
              v[ipiv] = vn;
            ...
            if (b_nlast > 0) {
              ipiv = (ia + ngroupsof5 * 5) + 1;
              ipiv = c_thirdOfFive(v, ipiv, (ipiv + b_nlast) - 1) - 1;
              ifirst = ia + ngroupsof5;
              vn = v[ifirst];
              v[ifirst] = v[ipiv];
              v[ipiv] = vn;
...
float quickselect(float v[30], int n, int vlen, int *nfirst, int *nlast)
{
  float vn;
  int k;
  if (n > vlen) {
