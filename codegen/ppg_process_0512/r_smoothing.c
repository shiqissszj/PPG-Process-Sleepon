...
float r_smoothing(float inputR, unsigned int b_outputCounter)
{
  ...
  if (b_outputCounter >= 30U) {
    RSmoothBuffer2[(int)b_qY - 1] = median(RSmoothBuffer1);
    ...
    RSmoothBuffer2[(int)b_qY - 1] = b_median(tmp_data, (int)b_outputCounter);
    ...
    outputR = b_median(tmp_data, (int)b_outputCounter);
    /*  For initial values when buffer is not full */
  }
