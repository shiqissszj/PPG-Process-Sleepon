...
float b_median(const float x[30])
{
  float v[30];
  float y;
  int a__3;
  ...
  do {
    ...
    if (k < 30) {
      ...
      y = b_quickselect(v, 16, 30, &k, &ilast);
      if (k > 15) {
        float b;
        b = b_quickselect(v, 15, ilast - 1, &k, &a__3);
...
float median(const float x_data[], int x_size)
{
  float a__4_data[49];
  float y;
  int j2;
      ...
      if (k <= x_size - 1) {
        ...
        if (x_size <= 4) {
          ...
          if (((unsigned int)x_size & 1U) == 0U) {
            ...
            y = quickselect(a__4_data, midm1 + 1, x_size, &k, &j3);
            if (midm1 < k) {
              float b;
              b = quickselect(a__4_data, midm1, j3 - 1, &k, &j2);
              if (((y < 0.0F) != (b < 0.0F)) || rtIsInfF(y)) {
                y = (y + b) / 2.0F;
              } else {
                y += (b - y) / 2.0F;
            ...
            memcpy(&a__4_data[0], &x_data[0],
                   (unsigned int)x_size * sizeof(float));
            y = quickselect(a__4_data, midm1 + 1, x_size, &k, &j2);
          }
        }
        exitg1 = 1;
      }
