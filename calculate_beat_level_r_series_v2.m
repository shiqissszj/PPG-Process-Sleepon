function result = calculate_beat_level_r_series_v2(ppgR, ppgIR, ppgG, samplingRate, outputSamples, windowSeconds)
% Candidate validation-only SpO2 feature.
%
% Green is used only to locate beats. Each beat produces one red/IR AC/DC
% ratio, and every output window uses the median of recent beat ratios.
% This is intentionally separate from the production path so it can be
% compared against raw/classic/green-template R before changing firmware.
...
[pulseR, ~] = diagnostic_pulse_filter(ppgR, samplingRate);
[pulseIR, ~] = diagnostic_pulse_filter(ppgIR, samplingRate);
[pulseG, ~] = diagnostic_pulse_filter(ppgG, samplingRate);

greenPeaks = find_green_peaks_for_beats(pulseG, samplingRate);
[beatCenter, beatR, beatRedPI, beatIRPI, beatGreenPI, beatValid] = ...
    build_beat_features(ppgR, ppgIR, ppgG, pulseR, pulseIR, pulseG, greenPeaks, samplingRate);
...
for idx = 1:windowCount
    ...
    totalBeatCount = sum(inWindow);
    if totalBeatCount > 0
        result.validBeatRatio(idx) = result.beatCount(idx) / totalBeatCount;
    end

    if result.beatCount(idx) >= max(3, floor(windowSeconds / 2))
        result.r(idx) = finite_median(beatR(validInWindow));
        result.redPI(idx) = finite_median(beatRedPI(validInWindow));
        result.irPI(idx) = finite_median(beatIRPI(validInWindow));
        result.greenPI(idx) = finite_median(beatGreenPI(validInWindow));
    end
end

result.spo2 = calculate_spo2_v2(result.r);
...
function [pulse, baseline] = diagnostic_pulse_filter(inputSignal, samplingRate)
...
function peaks = find_green_peaks_for_beats(pulseG, samplingRate)

scale = robust_percentile_range(pulseG, 5, 95);
...
minHeight = max(0.03, 0.15 * robust_percentile_range(normalizedG, 50, 90));

positivePeaks = find_local_peaks_with_distance(normalizedG, minDistance, minHeight);
negativePeaks = find_local_peaks_with_distance(-normalizedG, minDistance, minHeight);

if peak_train_score(negativePeaks, samplingRate) > peak_train_score(positivePeaks, samplingRate)
    peaks = negativePeaks;
else
    peaks = positivePeaks;
end
...
function peaks = find_local_peaks_with_distance(inputSignal, minDistance, minHeight)
...
function score = peak_train_score(peaks, samplingRate)
...
function [beatCenter, beatR, beatRedPI, beatIRPI, beatGreenPI, beatValid] = ...
    build_beat_features(ppgR, ppgIR, ppgG, pulseR, pulseIR, pulseG, peaks, samplingRate)
...
for idx = 2:(numel(peaks) - 1)
    ...
    redDc = finite_median(ppgR(beatRange));
    irDc = finite_median(ppgIR(beatRange));
    greenDc = finite_median(ppgG(beatRange));
    redAc = robust_percentile_range(pulseR(beatRange), 5, 95);
    irAc = robust_percentile_range(pulseIR(beatRange), 5, 95);
    greenAc = robust_percentile_range(pulseG(beatRange), 5, 95);
...
function output = finite_median(inputValues)
...
function output = robust_percentile_range(inputValues, lowPercentile, highPercentile)
...
lowValue = percentile_from_sorted(inputValues, lowPercentile);
highValue = percentile_from_sorted(inputValues, highPercentile);
output = highValue - lowValue;
end

function output = percentile_from_sorted(sortedValues, percentile)
