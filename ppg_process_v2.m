function [outputFlag, outputPR, outputSpO2, outputPI, confidenceR, rawR, fixedRDebug, smoothedRDebug, classicRDebug, redAcDcDebug, irAcDcDebug, qualityDebug] = ppg_process_v2(inputSampleR,inputSampleIR,inputSampleG,inputCounter,bodyMove)
...
if outputFlag
    ...
    [rawR, PR, PI, confidenceR, confidenceG, classicRDebug, redAcDcDebug, irAcDcDebug, qualityDebug] = r_pr_calculation_v2(windowR, windowIR, windowG, samplingRate, outputCounter, bodyMove);

    % fix the R and PR value based on the confidence
    [fixedR, fixedPR, confidenceR] = r_pr_fix_v2(rawR,PR,confidenceR,confidenceG,outputCounter);

    % smooth the R and PR value
    smoothedR = r_smoothing_v2(fixedR,outputCounter);
    smoothedPR = pr_smoothing_v2(fixedPR,outputCounter);
    fixedRDebug = fixedR;
    smoothedRDebug = smoothedR;

    outputPR = round(smoothedPR);
    outputSpO2 = calculate_spo2_v2(smoothedR);
    outputPI = PI;
else
    outputPR = single(0);
    outputSpO2 = single(0);
