function [outputR, outputPR, outputSQI, outputConfidenceR, outputConfidenceG, outputClassicR, outputRedAcDc, outputIRAcDc, outputQualityDebug] = r_pr_calculation_v2(windowR, windowIR, windowG, samplingRate, outputCounter, bodyMove)
...
function [outputR, isValid, redAcDc, irAcDc] = calculate_classic_r_protected(ppgFilteredR, ppgFilteredIR, dcR, dcIR)

acRValue = sqrt(mean(ppgFilteredR .^ 2));
acIRValue = sqrt(mean(ppgFilteredIR .^ 2));
dcRValue = mean(dcR);
dcIRValue = mean(dcIR);
...
guardAC = single(1e-6);
guardDC_R = single(1e-6) + single(1e-3) * abs(dcRValue);
guardDC_IR = single(1e-6) + single(1e-3) * abs(dcIRValue);

isValid = isfinite(acRValue) && isfinite(acIRValue) && ...
    isfinite(dcRValue) && isfinite(dcIRValue) && ...
    acRValue > guardAC && acIRValue > guardAC && ...
    abs(dcRValue) > guardDC_R && abs(dcIRValue) > guardDC_IR;
...
redAcDc = acRValue / dcRValue;
irAcDc = acIRValue / dcIRValue;
outputR = redAcDc / irAcDc;
isValid = isfinite(outputR);
if ~isValid
    outputR = single(NaN);
    redAcDc = single(NaN);
